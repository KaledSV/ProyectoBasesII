...
BEGIN
	DROP TEMPORARY TABLE IF EXISTS TempDirecciones;
	create temporary table  TempDirecciones (id INT NOT NULL AUTO_INCREMENT PRIMARY KEY,
										ejeX FLOAT,
                                        ejeY FLOAT);
	
    INSERT INTO TempDirecciones(ejeX,
								ejeY)
	SELECT ST_X(d.localizacion),
			ST_Y(d.localizacion)
    FROM entrega e
    INNER JOIN venta v ON v.id = e.id_venta
    INNER JOIN direccion d ON d.id_cliente = v.id_cliente
    WHERE e.entregado = 0;
...
-- Dump completed on 2021-06-23  0:47:38
